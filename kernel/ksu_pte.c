...
static int ksu_make_rw_section(unsigned long addr)
{
	...
	ksu_syms.aarch64_insn_write((void *)((unsigned long)pmd + 4),
				    new_val >> 32);
...
static int ksu_make_rw_pte_internal(unsigned long addr)
{
	...
	ksu_syms.aarch64_insn_write((void *)((unsigned long)pte + 4),
				    pte_val(new_pte) >> 32);
...
int ksu_make_exec_pte(unsigned long addr)
{
	...
	if (ksu_pmd_is_section(*pmd)) {
		...
		local_irq_save(flags);
		ksu_syms.aarch64_insn_write((void *)pmd,
					    pmd_val_new & 0xFFFFFFFF);
		ksu_syms.aarch64_insn_write((void *)((unsigned long)pmd + 4),
					    pmd_val_new >> 32);
		flush_tlb_kernel_range(addr & ~((1UL << 21) - 1),
				       (addr & ~((1UL << 21) - 1)) +

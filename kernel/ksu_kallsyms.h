...
struct ksu_symbols {
	...
	int (*__hashtab_insert)(void *h, void *key, void *datum,
				void *key_params);
	...
	int (*switch_task_namespaces)(struct task_struct *tsk,
				      struct nsproxy *new);
	...
	long (*strncpy_from_user_nofault)(char *dst, const void __user *src,
					  long count);

	/* Task work */
	int (*task_work_add)(struct task_struct *task,
			     struct callback_head *work, int notify);
	...
	long (*copy_from_user_nofault)(void *dst, const void __user *src,
				       size_t size);
	long (*copy_to_user_nofault)(void __user *dst, const void *src,
				     size_t size);
	...
	void (*change_pid)(struct task_struct *task, enum pid_type,
			   struct pid *pid);
};

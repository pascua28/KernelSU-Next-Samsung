...
static int KSU_NO_CFI disable_kprobe_blacklist(void)
{
	...
	kprobe_blacklist =
		(struct list_head *)ksu_early_lookup("kprobe_blacklist");
	if (!kprobe_blacklist) {
...
int KSU_NO_CFI ksu_early_cfi_bypass(void)
{
	...
	if (addr) {
		...
		if (*p != ARM64_BTI_C) {
			/* bti c; mov x0, #1; ret */
			ksu_call_insn_write(early_insn_write, p, ARM64_BTI_C);
			ksu_call_insn_write(early_insn_write, p + 1,
					    0xD2800020); /* mov x0, #1 */
...
void ksu_patch_cfi_functions(void)
{
	...
	const char *cfi_names[] = { "__cfi_slowpath",
				    "__cfi_slowpath_diag",
				    "_cfi_slowpath",
				    "__cfi_check_fail",
	...
	for (i = 0; i < 7; i++) {
		...
		if (current_insn == ARM64_BTI_C) {
			if (i == 6 &&
			    next_insn == 0xD2800020) { /* report_cfi_failure */
		...
		if (i == 6) { /* report_cfi_failure */
			ksu_call_insn_write((void *)ksu_syms.aarch64_insn_write,
					    (void *)cfi_funcs[i], ARM64_BTI_C);
			ksu_call_insn_write((void *)ksu_syms.aarch64_insn_write,
					    (void *)(cfi_funcs[i] + 4),
					    0xD2800020); /* mov x0, #1 */
			ksu_call_insn_write((void *)ksu_syms.aarch64_insn_write,
